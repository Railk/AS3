...
package railk.as3.painting {
	...
	import railk.as3.painting.BrushCreator;
	import railk.as3.painting.BrushState;	
	import railk.as3.painting.FlowEngineEvent;
	
	
	public class FlowEngine extends UISprite
	{	
		 ...
		 */
		public function FlowEngine( bgColor:uint, W:int, H:int, engineMode:String, layers:int = 1 ) {	
			_W = W;
			_H = H;
			_bgColor = bgColor
			init();
		...
		private function stopPaint( evt:MouseEvent ):void
		{
			...
			layer.bitmapData.draw( buffer,null,null,null,null,true );
			//--
			buffer.alpha = 0;
		...
		private function beginReplay( evt:TimerEvent ):void 
		{
			...
			if ( brushSteps < points.length )
			{
				...
				{
					layer.bitmapData.draw( buffer, m, null, null, null, true );
					//--
					buffer.alpha = 0;
					buffer.bitmapData.fillRect( buffer.bitmapData.rect, 0x00FFFFFF );
					BrushCreator.init( _W, _H, buffer.bitmapData );
		...
		// 																						   DRAW BRUSH
		// ——————————————————————————————————————————————————————————————————————————————————————————————————
		private function drawBrush( A:Point, B:Point, byteArrayDraw:Boolean = false, byteArray:ByteArray = null ):void 
		{
			var x:int = A.x;
			var y:int = A.y;
			var dx:int = B.x - A.x;
			...
			if ( (sqrDist > (BrushState.radius*.33 )) || BrushState.size < 40 )
			{
				...
				if (yLonger) {
					if (dx>0) {
						...
						for (var j:int = 0x8000 + (x << 16); y <= dx; y++) {
							buffer.bitmapData = BrushCreator.circleBrush( buffer.bitmapData,j >> 16,y,BrushState.radius,BrushState.color,BrushState.type,byteArrayDraw,byteArray );
							j+=decInc;
						}
					}
					dx+=y;
				...
				if (dx>0) {
					...
					for (j = 0x8000 + (y << 16); x <= dx; x++) {
						buffer.bitmapData = BrushCreator.circleBrush( buffer.bitmapData,x,j >> 16,BrushState.radius,BrushState.color,BrushState.type,byteArrayDraw,byteArray );
						j+=decInc;
					}
				}
				dx+=x;
			...
			{
				buffer.bitmapData = BrushCreator.circleBrush( buffer.bitmapData,x,y,BrushState.radius,BrushState.color,BrushState.type,byteArrayDraw,byteArray );
			}
		}
		...
		// 																				  	PARSE SMALL BRUSH
		// ——————————————————————————————————————————————————————————————————————————————————————————————————
		public function parseSmallBrushes( brushes:Array ):void {
			BrushCreator.parseSmallBrushes( brushes );
		}
